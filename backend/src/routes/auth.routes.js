...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (!username || !email || !password) return res.status(400).json({ error: 'Some fields are missing' });
    const exists = await User.findOne({ email });
    if (exists) return res.status(409).json({ error: 'This email is taken. Please try another one.' });
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    if (!ok) return res.status(401).json({ error: 'Incorrect email or password. Please try again.' });
